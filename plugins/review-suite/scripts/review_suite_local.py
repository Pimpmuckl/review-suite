...
def build_correctness_review_contract() -> str:
    return (
        ...
        "A correctness finding must identify a violated requirement, invariant, owner/source of truth, existing supported behavior, or explicit integration contract.\n"
        ...
        "If a concern depends on ambiguous product scope, report it under `Scope questions (non-findings)` instead of assigning severity. Do not propose defensive guards for scope assumptions.\n"
...
def build_manual_review_prompt(*, instructions: str, diff_text: str) -> str:
    ...
    return (
        ...
        "Report only concrete correctness findings. Product-scope assumptions are non-findings; put unresolved scope questions in a separate non-finding note. If there are no correctness issues, say 'No findings.'\n\n"
        "=== BEGIN DIFF ===\n"
        f"{diff_body}\n"
        "=== END DIFF ==="
    )
